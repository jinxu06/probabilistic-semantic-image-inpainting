...
from models.conv_vae_iwae import ConvVAEIWAE
from masks import get_generator
from configs import get_config

from learners.conv_vae_iwae_learner import ConvVAEIWAELearner
...
learner = ConvVAEIWAELearner(args.nr_gpu, args.save_dir, args.img_size, args.z_dim, exp_name=args.exp_name)
learner.load_data(args.data_set, args.batch_size*args.nr_gpu, use_debug_mode=args.debug)
learner.construct_models(model_cls=ConvVAEIWAE, model_opt=model_opt, learning_rate=args.learning_rate, trainable_params = ["inference"], eval_keys=['total loss', 'bits per dim'])
#
...
with tf.Session(config=config) as sess:
    ...
    if args.mode == 'train':
        ...
        if 'mnist' in args.data_set:
            learner.preload(from_dir="/data/ziz/jxu/save_dirs/model_mnist_conv_vae_{0}".format(mtype), var_list=None)
        elif 'celeba' in args.data_set:
            learner.preload(from_dir="/data/ziz/jxu/save_dirs/model_celeba_conv_vae_{0}".format(mtype), var_list=None)
        elif 'church_outdoor' in args.data_set:
            learner.preload(from_dir="/data/ziz/jxu/save_dirs/model_church_outdoor_conv_vae_{0}".format(mtype), var_list=None)
