...
class PixelCNN(object):
    ...
    def __model(self):
        ...
        with arg_scope([forward_pixel_cnn, reverse_pixel_cnn], **kwargs):
            inputs = self.x_bar
            self.pixel_params = forward_pixel_cnn(inputs, None, bn=False)
            if self.network_type == 'binary':
...
@add_arg_scope
def reverse_pixel_cnn_32(x, masks, nr_logistic_mix=10, nr_resnet=1, nr_filters=100, dropout_p=0.0, nonlinearity=None, bn=True, kernel_initializer=None, kernel_regularizer=None, is_training=False, counters={}):
    name = get_name("reverse_pixel_cnn_32", counters)
    ...
    with tf.variable_scope(name):
        with arg_scope([gated_resnet], gh=None, sh=None, nonlinearity=nonlinearity, dropout_p=dropout_p):
            with arg_scope([gated_resnet, up_shifted_conv2d, up_left_shifted_conv2d, up_shifted_deconv2d, up_left_shifted_deconv2d], bn=bn, kernel_initializer=kernel_initializer, kernel_regularizer=kernel_regularizer, is_training=is_training, counters=counters):
                ...
                u_list.append(up_shifted_conv2d(u_list[-1], num_filters=nr_filters, strides=[2, 2]))
                ul_list.append(up_left_shifted_conv2d(ul_list[-1], num_filters=nr_filters, strides=[2, 2]))
                ...
                for rep in range(nr_resnet):
                    u = gated_resnet(u, u_list.pop(), conv=up_shifted_conv2d)
                    ul = gated_resnet(ul, tf.concat([u, ul_list.pop()],3), conv=up_left_shifted_conv2d)

                u = up_shifted_deconv2d(u, num_filters=nr_filters, strides=[2, 2])
                ul = up_left_shifted_deconv2d(ul, num_filters=nr_filters, strides=[2, 2])
                ...
                x_out = nin(tf.nn.elu(ul), nr_filters)
                assert len(u_list) == 0
                assert len(ul_list) == 0
                return x_out
...
@add_arg_scope
def reverse_pixel_cnn_28_binary(x, masks, nr_logistic_mix=10, nr_resnet=1, nr_filters=100, dropout_p=0.0, nonlinearity=None, bn=True, kernel_initializer=None, kernel_regularizer=None, is_training=False, counters={}):
    name = get_name("reverse_pixel_cnn_28", counters)
    x = x * broadcast_masks_tf(masks, num_channels=3)
    x = tf.concat([x, broadcast_masks_tf(masks, num_channels=1)], axis=-1)
    print("construct", name, "...")
    print("    * nr_resnet: ", nr_resnet)
