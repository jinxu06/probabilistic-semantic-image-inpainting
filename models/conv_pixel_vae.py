...
class ConvPixelVAE(object):
    ...
    def __model(self, network_type="large"):
        ...
        if self.network_type == 'binary':
            ...
            num_channels = 3
        self.x = tf.placeholder(tf.float32, shape=(self.batch_size, self.img_size, self.img_size, num_channels))
        self.x_bar = tf.placeholder(tf.float32, shape=(self.batch_size, self.img_size, self.img_size, num_channels))
        ...
        with arg_scope([forward_pixelcnn, reverse_pixelcnn, encoder, decoder], **kwargs):
            ...
            with arg_scope([forward_pixelcnn, reverse_pixelcnn], **kwargs_pixelcnn):
                ...
                self.z_pr = gaussian_sampler(tf.zeros_like(self.z_mu), tf.ones_like(sigma))
                ...
                use_prior = tf.cast(tf.cast(self.use_prior, tf.int32), tf.float32)
                use_z_ph = tf.cast(tf.cast(self.use_z_ph, tf.int32), tf.float32)
                z = (use_prior * self.z_pr + (1-use_prior) * self.z) * (1-use_z_ph) + use_z_ph * self.z_ph
                ...
                self.mix_logistic_params = forward_pixelcnn(self.x_bar, cond_features, bn=False)
    ...
    def __loss(self):
        print("******   Compute Loss   ******")
        self.loss_nll = mix_logistic_loss(self.x, self.mix_logistic_params, masks=self.masks, output_mean=False)
        ...
        self.bits_per_dim = tf.reduce_mean(bits_per_dim_tf(nll=self.loss_nll, dim=tf.reduce_sum(1-self.masks, axis=[1,2])*3))
...
@add_arg_scope
def reverse_pixel_cnn_32_small(x, masks, nr_logistic_mix=10, nr_resnet=1, nr_filters=100, dropout_p=0.0, nonlinearity=None, bn=True, kernel_initializer=None, kernel_regularizer=None, is_training=False, counters={}):
