...
def read_imgs(dir, limit=-1):
    imgs = np.load(dir)['X'].reshape([-1, 28, 28, 1]).astype(np.float32)
    return imgs

def load(data_dir, subset='train', size=28, limit=-1):
    if subset in ['train', 'valid', 'test']:
        trainx = read_imgs(os.path.join(data_dir, "binarized-mnist-{0}.npz".format(subset)), limit=limit)
