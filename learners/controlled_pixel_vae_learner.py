...
class ControlledPixelVAELearner(LatentModelLearner):
    ...
    def examine_one_stage(self, mgen):
        self.restore()
        image = self.test_set.next()[7]
        z_mu, z_sigma, ori_data, masked_data, masks_np = self._examine_posterior(image, mgen)
        d = 0
        z_mu = z_mu[:5, d]
        z_sigma = z_sigma[:5, d]
        ori_data = ori_data[:5]
        ...
        f = "examine_one_stage"
        np.savez(f, z_mu=z_mu, z_sigma=z_sigma, ori=ori_data, masked=masked_data, masks_np=masks_np)

    def examine_two_stage(self, mgen):
        self.restore()
        image = self.test_set.next()[7]
        masks_np = np.load("examine_one_stage.npz")['masks_np']
        z_mu, z_sigma, ori_data, masked_data, masks_np = self._examine_posterior(image, mgen, masks_np=masks_np)
        d = 0
        z_mu = z_mu[:5, d]
        z_sigma = z_sigma[:5, d]
        ori_data = ori_data[:5]
    ...
    def examine_reg(self, mgen, image_id=0):
        ...
        visualize_samples(sample_x, os.path.join("results",'examine_%s_sample_%d.png' % (self.data_set, image_id)), layout=(8, 4), vrange=self.vrange)
    ...
    def inpainting(self, eval_mgen, layout=(5,5), same_inputs=False, use_mask_at=None):
        self.restore()
        #
        data = next(self.test_set)
        self.test_set.reset()
        ori_x, masked_x, sample_x = self.sample(data, eval_mgen, same_inputs=same_inputs, use_mask_at=use_mask_at)
        visualize_samples(ori_x, os.path.join("results", self.exp_name, 'inpainting_%s_gt.png' % (self.data_set)), layout=layout, vrange=self.vrange)
        visualize_samples(masked_x, os.path.join("results", self.exp_name, 'inpainting_%s_masked.png' % (self.data_set)), layout=layout, vrange=self.vrange)
        visualize_samples(sample_x, os.path.join("results", self.exp_name, 'inpainting_%s_sample.png' % (self.data_set)), layout=layout, vrange=self.vrange)
    ...
    def inspect(self, image_id=0, num_traversal_step=None):
        self.restore()
        data = next(self.test_set, image_id+1)[image_id]
        self.test_set.reset()
        if num_traversal_step is None:
            num_traversal_step = self.num_traversal_step
        sample_x = self.latent_inspect(data, num_traversal_step=num_traversal_step)
        visualize_samples(sample_x, os.path.join("results",'inspect_%s_sample_%d.png' % (self.data_set, image_id)), layout=(self.z_dim, num_traversal_step), vrange=self.vrange)
